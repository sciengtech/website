...
import type { CatalogProduct, SolutionContent } from '@shared/types';
import {
  CATEGORIES,
  PAGE_TEMPLATES,
  SOLUTION_GROUPS,
...
import { emptySolutionContent } from '@/lib/product-factory';
...
function ensureSolutionContent(content: SolutionContent | null | undefined): SolutionContent {
  return content ? { ...emptySolutionContent(), ...content } : emptySolutionContent();
}
...
export function ProductForm({
  ...
  const [draft, setDraft] = useState(() => ({
    ...
    solutionContent:
      product.pageTemplate === 'solution' || product.type === 'solution'
        ? ensureSolutionContent(product.solutionContent)
        : product.solutionContent,
  }));
  ...
  function setSolutionContent(patch: Partial<SolutionContent>) {
    setDraft((d) => ({
      ...d,
      solutionContent: { ...ensureSolutionContent(d.solutionContent), ...patch },
    }));
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const next: CatalogProduct = {
        ...
        id,
        images: draft.images || [],
        aliases: (draft.aliases || []).map((a) => String(a).trim()).filter(Boolean),
        customNote: draft.customNote?.trim() ? draft.customNote.trim() : null,
        ...
        solutionContent:
          draft.pageTemplate === 'solution' || draft.type === 'solution'
            ? ensureSolutionContent(draft.solutionContent)
            : draft.solutionContent,
      };
      await onSave(next);
    } catch (err) {
  ...
  const isSolutionPage = draft.pageTemplate === 'solution';
  const isConfigurable = draft.pageTemplate === 'configurable';
  const isVariantCatalog = draft.pageTemplate === 'variant-catalog';
  const showRfqEditor = isSolutionPage || isConfigurable;
  const showSolutionFields = isSolutionPage || (isConfigurable && isSolution);
    ...
    >
      <div className="space-y-6">
        <div className="grid gap-4 md:grid-cols-2">
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setDraft((d) => ({
                  ...
                  ...(tpl === 'solution' || (tpl === 'configurable' && d.type === 'solution')
                    ? { solutionContent: ensureSolutionContent(d.solutionContent) }
                    : {}),
                }));
              }}
            >
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">
              Lower numbers appear first on the live category page. Or drag rows in the list (Manual
              order).
            </p>
          </div>
          {isSolution ? (
        ...
        <div>
          ...
          {isRichPage && (
            <p className="mt-1 text-xs text-slate-500">
              Used on catalog cards and search — not shown in the page body.
            </p>
          )}
        </div>
        {!isRichPage && !isSolutionPage && (
          <div>
            <Label>Spec highlight</Label>
            <Textarea
              rows={2}
              value={draft.specHighlight}
              onChange={(e) => set('specHighlight', e.target.value)}
            />
            <p className="mt-1 text-xs text-slate-500">
              Shown under the SKU on the product page (hidden for solution pages that use a
              tagline).
            </p>
          </div>
        )}
        <div>
          <Label>Aliases</Label>
          <TagsInput value={draft.aliases || []} onChange={(aliases) => set('aliases', aliases)} />
          <p className="mt-1 text-xs text-slate-500">Shown under the product name on the live page.</p>
        </div>
        <div>
        ...
        {isRichPage && (
          <div>
            ...
            <p className="mb-2 text-xs text-slate-500">
              WordPress-style editor — fonts, colors, links, lists, tables, and images. Content
              appears below the product name and SKU on the live page.
            </p>
        ...
        {showSolutionFields && (
          <>
            <div>
              <Label>Tagline</Label>
              <Input
                value={draft.solutionContent?.tagline || ''}
                onChange={(e) => setSolutionContent({ tagline: e.target.value.trim() || null })}
                placeholder="Shown under the SKU on solution pages"
              />
            </div>
            <StringListEditor
              label="What this kit demonstrates"
              items={draft.solutionContent?.demonstrates || []}
              onChange={(v) => setSolutionContent({ demonstrates: v })}
            />
            <StringListEditor
              label="What's included"
              items={draft.solutionContent?.kitIncludes || []}
              onChange={(v) => setSolutionContent({ kitIncludes: v })}
            />
            <StringListEditor
              label="Key capabilities"
              items={draft.solutionContent?.capabilities || []}
              onChange={(v) => setSolutionContent({ capabilities: v })}
            />
          </>
        )}
          ...
          {showCustomTable && (
            <CustomTableEditor
              ...
              defaultTitlePlaceholder={
                isVariantCatalog ? 'TECHNICAL SPECIFICATIONS' : 'SPECIFICATION SHEET'
              }
        ...
        {!isRichPage && (
          <div>
            ...
            <Textarea
              ...
              placeholder="Optional note shown near CTAs / variants on the product page"
            />
          </div>
        )}
      ...
      {isRichPage && (
        <div>
          ...
          <div
            className="knowledge-prose min-h-[400px] rounded-md border border-[#2a3142] bg-[#161b26] p-4"
            dangerouslySetInnerHTML={{
              __html: draft.htmlBody || '<p class="text-slate-500">Start typing to preview…</p>',
            }}
          />
        </div>
      )}
